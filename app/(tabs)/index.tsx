  ...
  return (
    <View style={[styles.trustCard, polish.trustCard]}>
      <View style={styles.trustTopRow}>
        <View style={polish.trustIdentity}><View style={polish.trustAvatar}><Text style={polish.trustAvatarText}>RK</Text></View><View><Text style={polish.trustName}>Ramesh Kumar</Text><Text style={polish.trustRole}>Mason • Lucknow</Text></View></View>
      ...
      <View style={polish.scoreContext}><Text style={styles.trustLabel}>{c.score}</Text><View style={styles.trustScoreRow}><Text style={styles.trustScore}>87</Text><Text style={styles.trustOutOf}>/100</Text></View></View>
      <View style={styles.progressTrack}><View style={styles.progressFill87} /></View>
      <View style={polish.trustFooter}><AppIcon name="verified-user" size={15} color="#BED5F4" /><Text style={styles.trustDescription}>{c.verified}</Text></View>
    </View>
  );
}

function MetricCard({ icon, value, label, color }: { icon: string; value: string; label: string; color: string }) {
  const iconBackground = color === COLORS.success ? COLORS.successSoft : color === COLORS.orange ? COLORS.cream : "#EEF2F8";
  return <View style={[styles.metricCard, polish.metricCard]}><View style={[polish.metricIcon, { backgroundColor: iconBackground }]}><AppIcon name={icon} size={18} color={color} /></View><Text style={styles.metricValue}>{value}</Text><Text style={styles.metricLabel}>{label}</Text></View>;
}
...
export default function HomeScreen() {
  ...
  const renderPhone = () => <View style={styles.authScreen}><AuthHeader language={language} onLanguage={chooseLanguage} /><View style={[styles.authContent, polish.authContent]}><View style={polish.authIntro}><View style={polish.authProgressDot} /><Text style={styles.eyebrowOrange}>{c.digitalIdentity}</Text></View><Text style={styles.authTitle}>{c.phoneTitle}</Text><Text style={styles.authSubtitle}>{c.phoneHint}</Text><View style={polish.phoneTrustRow}><AppIcon name="shield" size={16} color={COLORS.success} /><Text style={polish.phoneTrustText}>{language === "Hindi" ? "Aapka number sirf secure verification ke liye use hoga" : "Your number is only used for secure verification"}</Text></View><View style={[styles.phoneField, polish.phoneField]}><Text style={styles.countryCode}>+91</Text><View style={styles.phoneDivider} /><TextInput accessibilityLabel="Mobile number" keyboardType="phone-pad" maxLength={10} onChangeText={(value) => setPhone(value.replace(/\D/g, ""))} placeholder="98765 43210" placeholderTextColor={COLORS.placeholder} returnKeyType="done" style={styles.phoneInput} value={phone} onSubmitEditing={continueToOtp} /></View><PrimaryButton label={c.continue} onPress={continueToOtp} /><View style={[styles.demoStrip, polish.demoStrip]}><AppIcon name="info-outline" size={18} color={COLORS.amberDark} /><Text style={styles.demoStripText}>{c.demo}: 9876543210 • OTP 1234</Text></View></View><Text style={styles.legalText}>{c.byContinuing}</Text></View>;
  const renderOtp = () => <View style={styles.authScreen}><Pressable accessibilityRole="button" onPress={() => setMode("phone")} style={({ pressed }) => [styles.backButton, pressed && styles.pressed]}><AppIcon name="arrow-back" size={20} /><Text style={styles.backText}>{c.back}</Text></Pressable><View style={[styles.otpContent, polish.otpContent]}><View style={polish.otpIconHalo}><View style={styles.lockBadge}><AppIcon name="lock" size={28} color={COLORS.orange} /></View></View><Text style={styles.authTitle}>{c.otp}</Text><Text style={styles.authSubtitle}>+91 {phone || "98765 43210"} {c.otpSent}</Text><Text style={polish.otpReassurance}>{language === "Hindi" ? "Bas 4 digits — aap almost ready hain." : "Just 4 digits — you are almost ready."}</Text><View style={styles.otpRow}>{otp.map((digit, index) => <TextInput accessibilityLabel={`OTP digit ${index + 1}`} autoFocus={index === 0} caretHidden key={`otp-${index}`} keyboardType="number-pad" maxLength={1} onChangeText={(value) => updateOtp(value, index)} onKeyPress={({ nativeEvent }) => { if (nativeEvent.key === "Backspace" && !otp[index] && index > 0) otpRefs.current[index - 1]?.focus(); }} ref={(input) => { otpRefs.current[index] = input; }} style={[styles.otpInput, polish.otpInput, digit ? styles.otpInputFilled : null]} value={digit} />)}</View><View style={[styles.demoStrip, polish.demoStrip]}><AppIcon name="auto-awesome" size={18} color={COLORS.orange} /><Text style={styles.demoStripText}>{c.demo} OTP: 1234</Text></View><PrimaryButton label={c.verify} onPress={verifyOtp} icon="verified" /><Text style={styles.resendText}>{c.otpHint}</Text></View></View>;

  const renderHome = () => <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}><PageHeader language={language} onLanguage={chooseLanguage} title={c.name} notification /><View style={polish.homePulse}><View><Text style={polish.homePulseTitle}>{language === "Hindi" ? "Aaj ka kaam, ek jagah." : "Your workday, in one place."}</Text><Text style={polish.homePulseText}>{language === "Hindi" ? "Lucknow • profile abhi updated hai" : "Lucknow • your profile is up to date"}</Text></View><View style={polish.liveDotWrap}><View style={polish.liveDot} /><Text style={polish.liveText}>{language === "Hindi" ? "Live" : "Live"}</Text></View></View><TrustCard c={c} /><View style={styles.metricsRow}><MetricCard icon="trending-up" value="₹950" label={c.earningsLabel} color={COLORS.success} /><MetricCard icon="shopping-cart" value="₹350" label={c.spendLabel} color={COLORS.orange} /><MetricCard icon="account-balance-wallet" value="₹600" label={c.todaySavings} color={COLORS.navy} /></View><Pressable accessibilityRole="button" onPress={() => { setActiveTab("Hisab"); haptic.light(); }} style={({ pressed }) => [styles.pendingBanner, polish.pendingBanner, pressed && styles.pressed]}><View style={styles.pendingIcon}><AppIcon name="schedule" size={21} color={COLORS.orange} /></View><View style={styles.pendingCopy}><Text style={styles.pendingTitle}>{c.payment}</Text><Text style={styles.pendingDescription}>{c.pendingPayment}</Text></View><View style={polish.pendingArrow}><AppIcon name="chevron-right" size={20} color={COLORS.muted} /></View></Pressable><View style={styles.sectionHeader}><View><Text style={polish.sectionEyebrow}>{language === "Hindi" ? "RECOMMENDED FOR YOU" : "PICKED FOR YOU"}</Text><Text style={styles.sectionTitle}>{c.bestMatch}</Text></View><Pressable onPress={() => setActiveTab("Kaam")} style={({ pressed }) => [styles.linkButton, pressed && styles.pressed]}><Text style={styles.linkText}>{c.jobsLabel}</Text></Pressable></View><JobCard job={jobFeed[0]} language={language} c={c} compact onApply={() => applyToJob(jobFeed[0])} /><View style={polish.quickActionsHeading}><Text style={styles.sectionTitle}>{language === "Hindi" ? "Aaj ke shortcuts" : "Today’s shortcuts"}</Text><Text style={polish.quickActionsHint}>{language === "Hindi" ? "Jo zaroori ho, wahi karein." : "Take care of what matters."}</Text></View><View style={[styles.quickActionsCard, polish.quickActionsCard]}><QuickAction icon="account-balance-wallet" label={c.fair} onPress={() => setActiveTab("Fair Wage")} /><QuickAction icon="add-circle-outline" label={c.addEntry} onPress={() => setEntryModalVisible(true)} /><QuickAction icon="work-outline" label={c.addJob} onPress={() => setJobModalVisible(true)} /><QuickAction icon="person-outline" label={c.profile} onPress={() => setActiveTab("Profile")} /></View></ScrollView>;
